...
/// Lightweight settings sheet — the privacy / community opt-in surface
/// plus a way to exercise right-to-erasure. Reachable from the gear
/// button in ContentView.
///
            ...
            Form {
                Section {
                    Toggle("Share anonymous cloud activity", isOn: $communityEnabled)
                        ...
                        .onChange(of: communityEnabled) { newValue in
                            ...
                            if newValue {
                                // User just opted in — this is the ONE place
                                // the iOS notification prompt fires (never at
                                // launch). Granting it registers with APNs,
                                // and the token flows to the backend via
                                // the consented registration path.
                ...
                Section("Privacy") {
                    ...
                    Button(role: .destructive) {
                        // Nothing is stored on-device anymore (drawings are
                        // share-in-the-moment only), so erasure = removing
                        // the device token from the backend.
                        notificationService.deleteFromBackend()
